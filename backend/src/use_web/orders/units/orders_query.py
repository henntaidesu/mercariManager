...
from .orders_helpers import _validate_status_query, _validate_time_field
...
def list_orders(
    ...
    ``time_field``：start_ts / end_ts 比较哪一列。缺省（或 ``updated``）= 最后更新优先
    COALESCE(order_updated_at, purchase_time, order_date)；``purchase`` = 购入时间；
    ``completed`` = 完成时间（completed_at，写一次不再变）。后两者不做回退，因此选「完成时间」时
    未完成的订单不会出现在任何区间里。
    """
    _validate_status_query(status)
    _validate_time_field(time_field)
